...
Chart templates are tested with
link:https://github.com/helm-unittest/helm-unittest[helm-unittest]. Test suites
live next to the chart they cover, in `+charts/<chart>/tests/*_test.yaml+`.

Install the plugin once (it is not available through `+asdf+`):

[source,bash]
----
helm plugin install --verify=false \
  https://github.com/helm-unittest/helm-unittest/releases/download/v1.1.2/unittest-1.1.2.tgz
----

The suites render the subcharts through the `+trento-server+` parent chart, so
its dependencies must be built beforehand:
