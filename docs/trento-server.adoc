...
Use a different log level (one of `+debug+`, `+info+`, `+warning+`, `+error+):
[source,shell]
....
helm install trento . --set global.logLevel="debug"
....

Use a different log level for a single component:
[source,shell]
....
helm install trento . --set trento-web.logLevel="debug" --set trento-wanda.logLevel="info"
....
